...
//Imports e Exports feitos para a tabela
...
export class FoodListComponent implements OnInit { displayedColumns: string[] = ['id', 'name', 'area', 'foodName', 'avaiableQuantity', 'price'];
...
function createNewUser(num: number): UserData {
const name = NAMES[Math.round(Math.random() * (NAMES.length - 1))] + ' ' +
    NAMES[Math.round(Math.random() * (NAMES.length - 1))].charAt(0) + '.';

var precision = 100; // 2 decimals
var price = Math.floor(Math.random() * (10 * precision - 1 * precision) + 1 * precision) / (1*precision);
